...
void PopulateBasicIpAddresses( dbo::Session& session ) {
  ...
  try {
    ...
    ptrIp = session.add( new DbRecIpAddress( ptrIana, "192.0.0.0/8", "reserved", "", "", "", sSource ) );
    ptrIp = session.add( new DbRecIpAddress( ptrIana, ptrIp, "192.0.2.0/24", "reserved", "RFC5737 TEST-NET-1", "", "http://tools.ietf.org/html/rfc5737", sSource ) );
    ptrIp = session.add( new DbRecIpAddress( ptrArin, ptrIp, "192.168.0.0/16", "reserved", "RFC1918", "", "http://tools.ietf.org/html/rfc1918", sSource ) );
    ptrIp = session.add( new DbRecIpAddress( ptrIana, ptrIp, "192.0.0.0/24", "reserved", "RFC5736", "", "http://tools.ietf.org/html/rfc5736", sSource ) );
    ptrIp = session.add( new DbRecIpAddress( ptrIana, "198.0.0.0/8", "reserved", "", "", "", sSource ) );
    ptrIp = session.add( new DbRecIpAddress( ptrIana, ptrIp, "198.18.0.0/15", "reserved", "RFC5735", "", "http://tools.ietf.org/html/rfc5735", sSource ) );
    ptrIp = session.add( new DbRecIpAddress( ptrIana, ptrIp, "198.51.100.0/24", "reserved", "RFC5735 TEST-NET-2", "", "http://tools.ietf.org/html/rfc5735", sSource ) );
